...
const DEFAULT_MEDS_FALLBACK: Medication[] = [
  {
    id: 'med-1',
    name: 'Lisinopril',
...
const DEFAULT_LOGS_FALLBACK: MedicationLog[] = [
  {
    id: 'log-101',
    medicationId: 'med-1',
...
export const MedicationModule: React.FC<MedicationModuleProps> = ({ userRole = 'caregiver', onLogUpdated }) => {
  const [medications, setMedications] = useState<Medication[]>(() => {
    const saved = localStorage.getItem('eldercare_medications_cache');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) {}
    }
    return DEFAULT_MEDS_FALLBACK;
  });

  const [logs, setLogs] = useState<MedicationLog[]>(() => {
    const saved = localStorage.getItem('eldercare_medication_logs_cache');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) {}
    }
    return DEFAULT_LOGS_FALLBACK;
  });
  ...
  useEffect(() => {
    try {
      localStorage.setItem('eldercare_medications_cache', JSON.stringify(medications));
    } catch (e) {}
  }, [medications]);

  useEffect(() => {
    try {
      localStorage.setItem('eldercare_medication_logs_cache', JSON.stringify(logs));
    } catch (e) {}
  }, [logs]);
  ...
  const handleLogEvent = async (medicationId: string, status: 'taken' | 'missed') => {
    ...
    try {
      ...
      console.warn('Network offline, applying local medication log update:', err);
    } finally {
    ...
    setMedications((prev) =>
      prev.map((m) =>
        m.id === medicationId
          ? { ...m, todayStatus: status, lastTakenAt: status === 'taken' ? nowTime : m.lastTakenAt }
          : m
      )
    );
  ...
  const handleAddMedication = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn('Network offline, adding medication locally:', err);
    } finally {
      setIsSubmittingForm(false);
    }

    // Local fallback
    setMedications((prev) => [...prev, newMedObj]);
    setName('');
    setDosage('');
    setInstructions('');
    setShowAddModal(false);
  ...
  const handleDeleteMedication = async (id: string) => {
    ...
    try {
      ...
      console.warn('Network offline, deleting medication locally:', err);
    }
    // Local fallback
