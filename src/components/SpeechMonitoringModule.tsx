...
export const SpeechMonitoringModule: React.FC<SpeechMonitoringModuleProps> = ({
  ...
  const [data, setData] = useState<SpeechMonitoringData | null>(null);
  ...
  const fetchMetrics = async () => {
    try {
      ...
      console.error('Error loading speech metrics:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRunAcousticTest = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/speech-metrics/sample', {
        ...
        body: JSON.stringify({
          speakingRate: 133 + Math.floor(Math.random() * 8) - 4,
          pauseDuration: Number((0.81 + (Math.random() * 0.1 - 0.05)).toFixed(2)),
          responseLatency: Number((1.12 + (Math.random() * 0.12 - 0.06)).toFixed(2)),
        }),
      });
      ...
      console.error('Error running acoustic test:', err);
    } finally {
