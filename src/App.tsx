  ...
  const handleSendMessage = async (text: string) => {
    try {
      ...
      try {
        ...
        console.error('Error talking to AI companion:', err);
        setIsProcessingAi(false);
      }
    } catch (globalErr) {
      ...
      {(currentView === 'role_selection' || currentView === 'auth') && (
        <LandingRoleSelection
          ...
          onSelectRole={(role, updatedProfile, updatedCheckIns) => {
            if (role === 'elderly' && !userAccount) {
              // Open linear 5-step registration wizard for new senior
              setOnboardingRole('elderly');
